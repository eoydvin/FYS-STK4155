...
class OLS(object):
    ...
    def fit(self, X, z):
        ...
        def c(beta, ytrue, x_design):
            return (1/ytrue.size)*(ytrue - x_design @ beta).T @ (ytrue - x_design @ beta) + self.lam * beta.T @ beta
        g = grad(c, 0)
        
        #test med å lage grad i loopen, kanskje det har med matriser å gjhere dette. 
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in Xz_split:
                ...
                #gr = (1.0/m) *batch[:, 0:-1].T @ ((batch[:, 0:-1] @ w) - batch[:, -1].reshape(-1, 1))
                gr = g(w, batch[:, -1].reshape(-1, 1), batch[:, 0:-1])
...
class Ridge(OLS):
    ...
    def fit(self, X, z):
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in Xz_split:
                g = (1.0/m) *batch[:, 0:-1].T @ ((batch[:, 0:-1] @ w) - batch[:, -1].reshape(-1, 1))
                if not np.isnan( np.sum(g) ):
